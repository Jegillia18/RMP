...
Left_HS_Index = Left_HS_Index';
Left_HS_Time(1:length(Left_HS_Time)) = 1;                                %Marks HS as 1 so I can keep track of events
Left_HS = [Left_HS_Time Left_HS_Index];


Left_TO_Time = Left_TO_Time';
Left_TO_Index = Left_TO_Index';
Left_TO_Time(1:length(Left_TO_Time)) = 0;                                %Marks TO as 0 so I can keep track of events
Left_TO = [Left_TO_Time Left_TO_Index];


Left_Cycle1 = [Left_HS; Left_TO];                                 %Creates an array with all the events of interests **Stacked
Left_Cycle_s = Left_Cycle1(:,2);

[temp, order] = sort(Left_Cycle_s(:,1) ,'ascend');                          %Sorts evetns in chronological order
Left_Cycle2 = [Left_Cycle1(order,1) temp];                                        %Sorts evetns in chronological order                            
...
Left_Cycle2 = Left_Cycle2(L_Start:end,:);                              % Performs first 25% cut off

% if Left_Cycle2(1,1) == 0                                                   %Checks to see if first event is TO. If yes go to next point
%     Left_Cycle2(1,:) = [];
% end

Left_Cycle = Left_Cycle2(1:L_middle_50,:);                             %Pulls data starting at new first point through the amount of points that is equal to 50% of the # number of events detected.
...
% What if we did Left ankle and just used cycle
Left_Ankle_Angle = Left_Cycle(:,2);
%Left_Peak_Dorsi = max(Left_Ankle_Angle);
...
Right_HS_Index = Right_HS_Index';
Right_HS_Time(1:length(Right_HS_Time)) = 1;                                %Marks HS as 1 so I can keep track of events
Right_HS = [Right_HS_Time Right_HS_Index];


Right_TO_Time = Right_TO_Time';
Right_TO_Index = Right_TO_Index';
Right_TO_Time(1:length(Right_TO_Time)) = 0;                                %Marks TO as 0 so I can keep track of events
Right_TO = [Right_TO_Time Right_TO_Index];


Right_Cycle1 = [Right_HS; Right_TO];                                 %Creates an array with all the events of interests **Stacked
Right_Cycle_s = Right_Cycle1(:,2);

[temp, order] = sort(Right_Cycle_s(:,1) ,'ascend');                          %Sorts evetns in chronological order
Right_Cycle2 = [Right_Cycle1(order,1) temp];                                        %Sorts evetns in chronological order                            
...
Right_Cycle2 = Right_Cycle2(L_Start:end,:);                              % Performs first 25% cut off

if Right_Cycle2(1,1) == 0                                                   %Checks to see if first event is TO. If yes go to next point
    Right_Cycle2(1,:) = [];
end

Right_Cycle = Right_Cycle2(1:L_middle_50,:);                             %Pulls data starting at new first point through the amount of points that is equal to 50% of the # number of events detected.

Right_Ankle_Angle = Right_Cycle(:,2);
%Left_Peak_Dorsi = max(Left_Ankle_Angle);
...
% data.(['Participant_',num2str(p),]).(['Trial_', num2str(t),]).(['WALK_', num2str(d),]).L = table(Left_Peak_Dorsi); 
% 
% data.(['Participant_',num2str(p),]).(['Trial_', num2str(t),]).(['WALK_', num2str(d),]).R = table(Right_Peak_Dorsi);
